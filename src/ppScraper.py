    ...
    for i, s in enumerate(states):
        ...
        for d in departments:
            ...
            try:
                ...
                newsroom_subtitle = newsroom_html.find(
                    "div", class_="newsroom-extra"
                ).text

                # weblinks
                newsroom_weblinks = newsroom_html.find(
                    "div", class_="newsroom-extra"
                ).find_all("a")
                newsroom_weblinks = [
...
def presseportal_crawler(
    ...
    list_of_newsroom_nr = (
        [newsroom_nr] if type(newsroom_nr) in [int, str] else newsroom_nr
    )
    ...
    for newsroom_counter, newsroom_nr in enumerate(list_of_newsroom_nr):
        ...
        for date in dates_to_scrape:
            ...
            while try_it == True:
                ...
                try:
                    # get daily html data
                    html_of_one_day = one_day_of_a_newsroom(
                        newsroom_nr, date, blaulicht
                    )
                    ...
                    for i, raw_html in enumerate(html_of_one_day):
                        ...
                        html_file_name = (
                            ...
                            + str(i)
                            + "_"
                            + str_scraping_datetime
                            + ".txt"
                        )

                        # Ordner für newsroom_nr anlegen
                        newsroom_year_data_folder_path = Path.joinpath(
                            year_data_folder_path, str(newsroom_nr)
                        )
                        utils.create_folder(newsroom_year_data_folder_path)

                        # check if article html already exists
                        html_files = [
                            html_file.rsplit("_", 1)[0]
                            for html_file in os.listdir(newsroom_year_data_folder_path)
                        ]
                        ...
                        if not update or (not duplicate and update):
                            file_path = Path.joinpath(
                                newsroom_year_data_folder_path, html_file_name
                            )
...
def scrape_blaulicht(
    ...
    state_data_folder = Path.joinpath(
        data_folder, "articles", "raw_article_html", state
    )
    ...
    if not isinstance(dept_df, pd.DataFrame):
        ...
        dept_df.to_csv(
            Path.joinpath(data_folder, "departments", dept_df_file_name), index=False
        )
